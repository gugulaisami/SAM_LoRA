...
def calc_loss(outputs, low_res_label_batch, ce_loss, dice_loss, dice_weight:float=0.8):
    low_res_logits = outputs['low_res_logits']# 获取低分辨率的logits
    loss_ce = ce_loss(low_res_logits, low_res_label_batch[:].long())# 计算交叉熵损失
    loss_dice = dice_loss(low_res_logits, low_res_label_batch, softmax=True)# 计算dice损失
    loss = (1 - dice_weight) * loss_ce + dice_weight * loss_dice
    return loss, loss_ce, loss_dice

#训练函数
def trainer_synapse(args, model, snapshot_path, multimask_output, low_res):
    from datasets.dataset_synapse import Synapse_dataset, RandomGenerator # 导入数据集和随机生成器
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    #从命令行中导出各项数据（学习率、类别数量等等）
